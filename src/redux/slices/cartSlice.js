...
export const removeFromCartapi = createAsyncThunk(
  ...
  async (item, { rejectWithValue, dispatch, getState }) => {
    ...
    try {
      if (auth?.isAuthenticated) {
        const data = {
          retailer_product_id: item?.retailer_id ? item?.retailer_product_id || item?.product_id : "",
          product_id: !item?.retailer_id ? item?.id || item?.product_id : "",
        };
        dispatch(removeFromCart(keyPayload));
        const response = await axiosInstance.post("/customer/remove-to-cart",data);
...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    removeFromCart: (state, action) => {
      const targetId = action.payload?.id ?? action.payload?.product_id ?? action.payload?.retailer_product_id;
      const targetVar = action.payload?.selected_variant || action.payload?.selectedVariant || null;
